...
async fn connect_pg(pg_url: &str, tls: &MakeTlsConnect) -> tokio_postgres::Client {
    // a couple of quick retries in case of transient DNS/network hiccups
...
pub async fn run(fetcher_running: Arc<AtomicBool>) {
    ...
    // Connect Postgres over TLS (with retries)
    ...
    loop {
        ...
        if elapsed < Duration::from_secs(FETCH_INTERVAL_SECS) {
            coop_sleep(
                &fetcher_running,
                Duration::from_secs(FETCH_INTERVAL_SECS) - elapsed,
            )
            .await;
        } else {
